...
import me.eskid.nhacks.features.modules.combat.Killaura;
import me.eskid.nhacks.features.modules.player.Blink;
import me.eskid.nhacks.features.modules.player.FakePlayer;
...
public class EntityUtil implements Util {
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            if (NHacksWare.friendManager.isFriend(player)) {
                continue;
            }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(mc.player) || (entity instanceof EntityPlayer && NHacksWare.friendManager.isFriend(entity.getName())) || mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && NHacksWare.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = NHacksWare.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (NHacksWare.friendManager.isFriend(player) ? "§b" : "§r") + player.getName() + " " + distanceSB.toString() + " " + "§f" + NHacksWare.totemPopManager.getTotemPopString(player) + NHacksWare.potionManager.getTextRadarPotion(player), (int) mc.player.getDistance(player));
