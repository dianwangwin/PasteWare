...
import me.eskid.nhacks.event.events.*;
import me.eskid.nhacks.features.Feature;
import me.eskid.nhacks.features.command.Command;
import me.eskid.nhacks.features.modules.client.Managers;
import me.eskid.nhacks.features.modules.client.ServerModule;
import me.eskid.nhacks.features.modules.combat.AutoCrystal;
import me.eskid.nhacks.util.GLUProjection;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            NHacksWare.potionManager.update();
            NHacksWare.totemPopManager.onUpdate();
            NHacksWare.inventoryManager.update();
            NHacksWare.holeManager.update();
            NHacksWare.safetyManager.onUpdate();
            NHacksWare.moduleManager.onUpdate();
            NHacksWare.timerManager.update();
            if (this.timer.passedMs(Managers.getInstance().moduleListUpdates.getValue().intValue())) {
                NHacksWare.moduleManager.sortModules(true);
                NHacksWare.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        NHacksWare.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        NHacksWare.moduleManager.onLogout();
        NHacksWare.totemPopManager.onLogout();
        NHacksWare.potionManager.onLogout();
    }
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        NHacksWare.moduleManager.onTick();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            NHacksWare.baritoneManager.onUpdateWalkingPlayer();
            NHacksWare.speedManager.updateValues();
            NHacksWare.rotationManager.updateRotations();
            NHacksWare.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            NHacksWare.rotationManager.restoreRotations();
            NHacksWare.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        NHacksWare.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                NHacksWare.totemPopManager.onTotemPop(player);
                NHacksWare.potionManager.onTotemPop(player);
            }
            ...
            NHacksWare.serverManager.update();
        }
    }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        EventManager.mc.profiler.startSection("nhacks");
        ...
        NHacksWare.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            NHacksWare.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            NHacksWare.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    NHacksWare.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
