...
import me.eskid.nhacks.features.modules.render.XRay;
import me.eskid.nhacks.features.setting.Bind;
import me.eskid.nhacks.features.setting.EnumConverter;
import me.eskid.nhacks.features.setting.Setting;
import me.eskid.nhacks.util.Util;
...
public class ConfigManager
    ...
    public String config = "nhacks/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                NHacksWare.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    NHacksWare.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("nhacks").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("nhacks/" + name + "/")) ? "nhacks/" + name + "/" : "nhacks/config/";
    ...
    public void saveConfig(String name) {
        ...
        this.config = "nhacks/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        NHacksWare.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("nhacks/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("nhacks", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(NHacksWare.moduleManager.modules);
        this.features.add(NHacksWare.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        NHacksWare.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            NHacksWare.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
