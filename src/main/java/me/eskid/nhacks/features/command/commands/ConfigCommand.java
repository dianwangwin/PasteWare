...
public class ConfigCommand
    ...
    public void execute(String[] commands) {
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save": {
                    NHacksWare.configManager.saveConfig(commands[1]);
                    ConfigCommand.sendMessage("\u00a7aConfig has been saved.");
                    break;
                }
                case "load": {
                    NHacksWare.moduleManager.onUnload();
                    NHacksWare.configManager.loadConfig(commands[1]);
                    NHacksWare.moduleManager.onLoad();
