...
public class Components
    ...
    public static EntityPlayer getClosestEnemy() {
        ...
        for (EntityPlayer player : Components.mc.world.playerEntities) {
            if (player == Components.mc.player || NHacksWare.friendManager.isFriend(player)) continue;
    ...
    public void drawTargetHud(float partialTicks) {
        if (this.design.getValue() == TargetHudDesign.NORMAL) {
            ...
            this.renderer.drawStringWithShadow("Pops: " + NHacksWare.totemPopManager.getTotemPops(target), this.targetHudX.getValue() + 60, this.targetHudY.getValue() + this.renderer.getFontHeight() * 2 + 30, ColorUtil.toRGBA(255, 0, 0, 255));
            GlStateManager.enableTexture2D();
            int iteration = 0;
