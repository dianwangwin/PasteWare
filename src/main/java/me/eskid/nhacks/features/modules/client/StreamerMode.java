...
import me.eskid.nhacks.util.TextUtil;
...
public class StreamerMode
    ...
    public void onUpdate() {
        if (this.window != null) {
            ...
            drawInfo.add("TPS: " + NHacksWare.serverManager.getTPS());
            drawInfo.add("Ping: " + NHacksWare.serverManager.getPing() + "ms");
            drawInfo.add("Speed: " + NHacksWare.speedManager.getSpeedKpH() + "km/h");
            ...
            String text = NHacksWare.rotationManager.getDirection4D(false);
            drawInfo.add("");
            drawInfo.add(text);
            drawInfo.add(coordinates);
            drawInfo.add("");
            for (Module module : NHacksWare.moduleManager.sortedModules) {
                String moduleName = TextUtil.stripColor(module.getFullArrayString());
                drawInfo.add(moduleName);
            }
            drawInfo.add("");
            for (PotionEffect effect : NHacksWare.potionManager.getOwnPotions()) {
                String potionText = TextUtil.stripColor(NHacksWare.potionManager.getColoredPotionString(effect));
