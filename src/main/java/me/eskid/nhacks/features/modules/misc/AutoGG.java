...
import me.eskid.nhacks.event.events.DeathEvent;
...
import me.eskid.nhacks.manager.FileManager;
...
public class AutoGG
        extends Module {
    private static final String path = "nhacks/autogg.txt";
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (event.getTarget() instanceof EntityPlayer && !NHacksWare.friendManager.isFriend(event.getEntityPlayer())) {
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        ...
        if (event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(AutoGG.mc.world) instanceof EntityPlayer && !NHacksWare.friendManager.isFriend((EntityPlayer) packet.getEntityFromWorld(AutoGG.mc.world))) {
