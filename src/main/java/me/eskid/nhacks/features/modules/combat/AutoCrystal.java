...
import me.eskid.nhacks.event.events.ClientEvent;
import me.eskid.nhacks.event.events.PacketEvent;
import me.eskid.nhacks.event.events.Render3DEvent;
import me.eskid.nhacks.event.events.UpdateWalkingPlayerEvent;
...
import me.eskid.nhacks.features.modules.client.Colors;
import me.eskid.nhacks.features.modules.misc.NoSoundLag;
import me.eskid.nhacks.features.setting.Bind;
import me.eskid.nhacks.features.setting.Setting;
import me.eskid.nhacks.util.Timer;

import me.eskid.nhacks.util.*;
...
public class AutoCrystal
    ...
    public final me.eskid.nhacks.util.Timer threadTimer = new me.eskid.nhacks.util.Timer();
    ...
    private final me.eskid.nhacks.util.Timer switchTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer manualTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer breakTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer placeTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer syncTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer predictTimer = new me.eskid.nhacks.util.Timer();
    private final me.eskid.nhacks.util.Timer renderTimer = new me.eskid.nhacks.util.Timer();
    private final AtomicBoolean shouldInterrupt = new AtomicBoolean(false);
    private final me.eskid.nhacks.util.Timer syncroTimer = new me.eskid.nhacks.util.Timer();
    private final Map<EntityPlayer, me.eskid.nhacks.util.Timer> totemPops = new ConcurrentHashMap<EntityPlayer, me.eskid.nhacks.util.Timer>();
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !NHacksWare.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity(AutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer) packet5.getEntity(AutoCrystal.mc.world), new me.eskid.nhacks.util.Timer().reset());
            }
    ...
    private boolean isRightThread() {
        return mc.isCallingFromMinecraftThread() || !NHacksWare.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof PhobosGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue().booleanValue() && this.offhandSwitch.getValue().booleanValue() && this.offHand) {
                Offhand module = NHacksWare.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !NHacksWare.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtil.calculateDamage(entity, player)) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
                    ...
                    if (syncflag || this.currentDamage - (double) damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue().intValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            if (this.foundDoublePop) {
                                this.totemPops.put(target, new me.eskid.nhacks.util.Timer().reset());
                            }
        ...
        for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !NHacksWare.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtil.calculateDamage(pos, friend)) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
        ...
        switch (this.rotate.getValue()) {
            ...
            case ALL: {
                ...
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    NHacksWare.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (NHacksWare.eventManager.ticksOngoing()) {
                    }
                    ...
                    NHacksWare.safetyManager.doSafetyCheck();
