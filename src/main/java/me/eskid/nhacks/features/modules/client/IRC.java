...
import me.eskid.nhacks.manager.WaypointManager;
...
public class IRC
    ...
    public void onRender3D(final Render3DEvent event) {
        ...
        if (this.waypoints.getValue()) {
            for (final WaypointManager.Waypoint waypoint : NHacksWare.waypointManager.waypoints.values()) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        if (this.inventories.getValue()) {
            ...
            this.textRadarY = 0;
            for (final String player : IRC.phobosUsers) {
                if (NHacksWare.inventoryManager.inventories.get(player) != null) {
                    continue;
                }
                final List<ItemStack> stacks = NHacksWare.inventoryManager.inventories.get(player);
                            ...
                            if (input.equalsIgnoreCase("list")) {
                                ...
                                for (final String friend : split2) {
                                    if (!friend.equals(Util.mc.player.getName())) {
                                        NHacksWare.friendManager.addFriend(friend);
                                ...
                                NHacksWare.waypointManager.waypoints.put(name, new WaypointManager.Waypoint(name, server, Integer.parseInt(dimension), Integer.parseInt(inputs[2]), Integer.parseInt(inputs[3]), Integer.parseInt(inputs[4]), color));
                                ...
                                final String name = this.inputStream.readUTF();
                                NHacksWare.waypointManager.waypoints.remove(name);
                                ...
                                for (final String player : IRC.phobosUsers) {
                                    if (player.equalsIgnoreCase(name)) {
                                        NHacksWare.inventoryManager.inventories.put(player, IRC.deserializeInventory(inventory));
                                    }
                                }
