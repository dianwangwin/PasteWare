package me.eskid.nhacks.features.gui;
...
import me.eskid.nhacks.features.gui.components.items.buttons.ModuleButton;
...
public class PhobosGui
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : NHacksWare.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    NHacksWare.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
