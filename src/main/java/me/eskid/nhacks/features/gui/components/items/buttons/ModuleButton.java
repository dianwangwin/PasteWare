...
public class ModuleButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            ClickGui gui = NHacksWare.moduleManager.getModuleByClass(ClickGui.class);
            NHacksWare.textManager.drawStringWithShadow(gui.openCloseChange.getValue().booleanValue() ? (this.subOpen ? gui.close.getValue() : gui.open.getValue()) : gui.moduleButton.getValue(), this.x - 1.5f + (float) this.width - 7.4f, this.y - 2.0f - (float) PhobosGui.getClickGui().getTextOffset(), -1);
            if (this.subOpen) {
                float height = 1.0f;
