...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            NHacksWare.textManager.drawStringWithShadow(this.currentString.getString() + NHacksWare.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            NHacksWare.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " " + "\u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
