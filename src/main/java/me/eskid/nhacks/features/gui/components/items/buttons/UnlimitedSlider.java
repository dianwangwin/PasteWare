...
public class UnlimitedSlider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ClickGui.getInstance().rainbowRolling.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? NHacksWare.colorManager.getColorWithAlpha(NHacksWare.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : NHacksWare.colorManager.getColorWithAlpha(NHacksWare.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        }
        NHacksWare.textManager.drawStringWithShadow(" - " + this.setting.getName() + " " + "\u00a77" + this.setting.getValue() + "\u00a7r" + " +", this.x + 2.3f, this.y - 1.7f - (float) PhobosGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
    }
